...
exports.generateDynamicKeyName = exports.encodeKeyName = exports.isDynamicKeyName = exports.encodeDynamicKeyPart = void 0;
 ...
 */
const encodeDynamicKeyPart = (type, value, bytes) => {
    let baseType = '';
    let size = 0;
    ...
    switch (baseType) {
        case 'string':
            return (0, web3_utils_1.keccak256)(value).slice(2, 2 + bytes * 2);
        case 'bool': {
            if (value !== 'true' && value !== 'false') {
                throw new Error(`Wrong value: ${value} for dynamic key with type: <bool>. Expected "true" or "false".`);
            }
            return (0, web3_utils_1.leftPad)(+(value === 'true'), bytes * 2).slice(2);
        }
        case 'address': {
            ...
            if (bytes > 20) {
                return (0, web3_utils_1.leftPad)(value.replace('0x', ''), bytes * 2).toLowerCase();
            }
            return value
                .replace('0x', '')
                .slice(0, bytes * 2)
        ...
        case 'uint': {
            ...
            const hexNumber = (0, web3_utils_1.numberToHex)(value).slice(2);
            if (hexNumber.length <= bytes * 2) {
                return (0, web3_utils_1.padLeft)(hexNumber, bytes * 2);
            }
        ...
        case 'bytes': {
            ...
            if (!(0, web3_utils_1.isHex)(valueWithoutPrefix)) {
            ...
            return (0, web3_utils_1.leftPad)(valueWithoutPrefix, bytes * 2).toLowerCase();
        }
        default:
            throw new Error(`Dynamic key: ${type} is not supported`);
    }
};
exports.encodeDynamicKeyPart = encodeDynamicKeyPart;
...
exports.isDynamicKeyName = isDynamicKeyName;
/**
...
const encodeDynamicMapping = (name, dynamicKeyParts) => {
    ...
    const encodedKey = (0, web3_utils_1.keccak256)(keyNameSplit[0]).slice(0, 22);
    return `${encodedKey}0000${(0, exports.encodeDynamicKeyPart)(keyNameSplit[1], dynamicKeyParts[0], 20)}`;
};
/**
 * Encodes a MappingWithGrouping with dynamic values, according to LSP-2 ERC725YJSONSchema.
...
const encodeDynamicMappingWithGrouping = (name, dynamicKeyParts) => {
    ...
    const firstPart = (0, web3_utils_1.keccak256)(keyNameSplit[0]).slice(0, 14);
    let secondPart = '';
    if (isDynamicKeyName(keyNameSplit[1])) {
        secondPart = (0, exports.encodeDynamicKeyPart)(keyNameSplit[1], dynamicKeyParts[0], 4);
    }
    else {
        secondPart = (0, web3_utils_1.keccak256)(keyNameSplit[1]).slice(2, 2 + 4 * 2);
    }
    let lastPart = '';
    if (isDynamicKeyName(keyNameSplit[2])) {
        lastPart = (0, exports.encodeDynamicKeyPart)(keyNameSplit[2], dynamicKeyParts[dynamicKeyParts.length - 1], 20);
    }
    else {
        lastPart = (0, web3_utils_1.keccak256)(keyNameSplit[2]).slice(2, 2 + 20 * 2);
    }
...
function encodeDynamicKeyName(name, dynamicKeyParts) {
    ...
    const keyType = (0, utils_1.guessKeyTypeFromKeyName)(name);
    switch (keyType) {
        case 'Mapping':
...
function encodeKeyName(name, dynamicKeyParts) {
    ...
    switch (keyType) {
        ...
        case 'Mapping': {
            const keyNameSplit = name.split(':');
            if ((0, web3_utils_1.isAddress)(keyNameSplit[1])) {
        ...
        case 'Singleton':
            return (0, web3_utils_1.keccak256)(name);
        default:
            return (0, web3_utils_1.keccak256)(name);
    }
}
exports.encodeKeyName = encodeKeyName;
const generateDynamicKeyName = (name, dynamicKeyParts) => {
    ...
    return name
        ...
        if (keyNamePart === '<address>') {
            if (!(0, web3_utils_1.isAddress)(dynamicKeyPart)) {
...
exports.generateDynamicKeyName = generateDynamicKeyName;
