...
const encodeDataSourceWithHash = (verification, dataSource) => {
    ...
    return [
        '0x0000',
        (0, web3_utils_1.stripHexPrefix)(verificationMethod
            ? (0, web3_utils_1.padLeft)((0, web3_utils_1.keccak256)(verificationMethod.name).slice(0, 10), 8)
            : '00000000'),
        (0, web3_utils_1.stripHexPrefix)((verification === null || verification === void 0 ? void 0 : verification.data)
            ? (0, web3_utils_1.padLeft)(verification.data.slice(2).length / 2, 4)
            : '0000'),
        (0, web3_utils_1.stripHexPrefix)((verification === null || verification === void 0 ? void 0 : verification.data) ? (0, web3_utils_1.stripHexPrefix)(verification === null || verification === void 0 ? void 0 : verification.data) : ''),
        (0, web3_utils_1.stripHexPrefix)((0, web3_utils_1.utf8ToHex)(dataSource)),
    ].join('');
};
const decodeDataSourceWithHash = (value) => {
    if (value.slice(0, 6) === '0x0000') {
        ...
        */
        ...
        const dataLength = (0, web3_utils_1.hexToNumber)(encodedLength, false);
...
const encodeToBytesN = (bytesN, value) => {
    ...
    if (typeof value === 'string' && !(0, web3_utils_1.isHex)(value)) {
        ...
        valueToEncode = (0, web3_utils_1.numberToHex)(value);
    }
    else {
        valueToEncode = value;
    }
    ...
    return (0, web3_utils_1.bytesToHex)(bytesArray.slice(0, 4));
};
/**
...
const encodeCompactBytesArray = (values) => {
    const compactBytesArray = values
        ...
        .reduce((acc, value) => {
        const numberOfBytes = (0, web3_utils_1.stripHexPrefix)(value).length / 2;
        const hexNumber = (0, web3_utils_1.padLeft)((0, web3_utils_1.numberToHex)(numberOfBytes), 4);
        return acc + (0, web3_utils_1.stripHexPrefix)(hexNumber) + (0, web3_utils_1.stripHexPrefix)(value);
    }, '0x');
...
const decodeCompactBytesArray = (compactBytesArray) => {
    if (!(0, web3_utils_1.isHex)(compactBytesArray))
        throw new Error("Couldn't decode, value is not hex");
    let pointer = 0;
    const encodedValues = [];
    const strippedCompactBytesArray = (0, web3_utils_1.stripHexPrefix)(compactBytesArray);
    while (pointer < strippedCompactBytesArray.length) {
        ...
        if (length === 0) {
            ...
            encodedValues.push('0x' +
                strippedCompactBytesArray.slice(pointer + 4, pointer + 2 * (length + 2)));
        }
        pointer += 2 * (length + 2);
    }
...
const encodeBytesNCompactBytesArray = (values, numberOfBytes) => {
    values.forEach((value, index) => {
        if ((0, web3_utils_1.stripHexPrefix)(value).length > numberOfBytes * 2)
...
const decodeBytesNCompactBytesArray = (compactBytesArray, numberOfBytes) => {
    ...
    bytesValues.forEach((bytesValue, index) => {
        if ((0, web3_utils_1.stripHexPrefix)(bytesValue).length > numberOfBytes * 2)
            throw new Error(`Hex bytes${numberOfBytes} value at index ${index} does not fit in ${numberOfBytes} bytes`);
    });
    return bytesValues;
};
...
const encodeUintNCompactBytesArray = (values, numberOfBytes) => {
    const hexValues = values.map((value, index) => {
        const hexNumber = (0, web3_utils_1.stripHexPrefix)((0, web3_utils_1.numberToHex)(value)).padStart(numberOfBytes * 2, '0');
...
const decodeUintNCompactBytesArray = (compactBytesArray, numberOfBytes) => {
    ...
    return hexValues.map((hexValue, index) => {
        const hexValueStripped = (0, web3_utils_1.stripHexPrefix)(hexValue);
        if (hexValueStripped.length > numberOfBytes * 2)
            throw new Error(`Hex uint${numberOfBytes * 8} value at index ${index} does not fit in ${numberOfBytes} bytes`);
        return Number((0, web3_utils_1.hexToNumber)(hexValue));
    });
};
/**
...
const valueTypeEncodingMap = (type) => {
    ...
    switch (type) {
        ...
        case `uint${uintLength}`:
            return {
                ...
                decode: (value) => {
                    ...
                    return (0, web3_utils_1.toBN)(value).toNumber();
                },
            };
        case 'bytes32':
            return {
                encode: (value) => encodeToBytesN('bytes32', value),
                decode: (value) => abiCoder.decodeParameter('bytes32', value),
            };
        case 'bytes4':
            return {
                encode: (value) => encodeToBytesN('bytes4', value),
                decode: (value) => {
        ...
        case 'bool[]':
            return {
                encode: (value) => abiCoder.encodeParameter('bool[]', value),
                decode: (value) => abiCoder.decodeParameter('bool[]', value),
            };
        case 'boolean[]':
            return {
                encode: (value) => abiCoder.encodeParameter('bool[]', value),
    ...
    switch (valueContent) {
        ...
        case 'Number': {
            return {
                ...
                decode: (value) => Number((0, web3_utils_1.hexToNumber)(value)),
            };
        }
