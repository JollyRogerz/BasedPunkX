...
exports.decodeData = exports.decodeKey = exports.decodeTupleKeyValue = exports.isValidTuple = void 0;
/**
...
const ERC725JSONSchema_1 = require("../types/ERC725JSONSchema");
...
const isValidTuple = (valueType, valueContent) => {
    ...
    const valueTypeToDecode = valueType.replace(constants_1.COMPACT_BYTES_ARRAY_STRING, '');
    ...
    for (let i = 0; i < valueTypeParts.length; i++) {
        if (!(0, ERC725JSONSchema_1.isValidValueType)(valueTypeParts[i])) {
            throw new Error(`Invalid tuple for valueType: ${valueType} / valueContent: ${valueContent}. Type: ${valueTypeParts[i]} is not valid. Valid types are: ${ERC725JSONSchema_1.ALL_VALUE_TYPES}`);
        }
        ...
        if ((0, web3_utils_1.isHexStrict)(valueContentParts[i])) {
...
value) => {
    ...
    valueTypeParts.forEach((valueTypePart) => {
        ...
        if (numericMatch)
            bytesLengths.push(parseInt(numericMatch[1], 10) / 8);
...
function decodeKey(schema, value) {
    ...
    switch (lowerCaseKeyType) {
        case 'array': {
            ...
            const arrayLength = (0, utils_1.decodeKeyValue)('Number', 'uint128', valueElement.value, schema.name) ||
                0;
        ...
        case 'mapping': {
            ...
            if (schema.valueType.includes(constants_1.COMPACT_BYTES_ARRAY_STRING)) {
                ...
                if (valueType[0] === '(' && valueType[valueType.length - 1] === ')') {
                    const decodedCompactBytesArray = (0, encoder_1.decodeValueType)('bytes[CompactBytesArray]', value);
                    return decodedCompactBytesArray.map((element) => (0, exports.decodeTupleKeyValue)(valueContent, valueType, element));
                }
                return (0, encoder_1.decodeValueType)(schema.valueType, value);
            }
...
function decodeData(data, schema) {
    const processDataInput = ({ keyName, dynamicKeyParts, value }, throwException = true) => {
        ...
        const schemaElement = isDynamic
            ? (0, getSchemaElement_1.getSchemaElement)(schema, keyName, dynamicKeyParts)
            : (0, getSchemaElement_1.getSchemaElement)(schema, keyName);
